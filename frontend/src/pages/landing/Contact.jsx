...
function validate(name, value, lang) {
  const msg = {
    es: {
      ...
      emailMax:        'El email no puede superar 150 caracteres',
      companyInvalid:  'La empresa contiene caracteres no permitidos',
      companyMax:      'El nombre de empresa no puede superar 150 caracteres',
      messageRequired: 'El mensaje es obligatorio',
      messageMin:      'El mensaje debe tener al menos 20 caracteres',
      messageMax:      'El mensaje no puede superar 2000 caracteres',
    },
    en: {
      ...
      companyInvalid:  'Company contains invalid characters',
    ...
    fr: {
      ...
      nameLetters:     'Le nom ne peut contenir que des lettres',
      nameMin:         'Le nom doit contenir au moins 2 caractères',
      nameMax:         'Le nom ne peut pas dépasser 100 caractères',
      emailRequired:   "L'email est obligatoire",
      emailInvalid:    'Veuillez entrer un email valide',
      emailMax:        "L'email ne peut pas dépasser 150 caractères",
      companyInvalid:  "L'entreprise contient des caractères non autorisés",
      companyMax:      "Le nom de l'entreprise ne peut pas dépasser 150 caractères",
...
function Contact() {
  ...
  function isFormValid() {
    return (
      !validate('name',    form.name,    lang) &&
      !validate('email',   form.email,   lang) &&
      !validate('message', form.message, lang)
    )
  }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // Demo mode: show success if backend not connected
      setStatus('sent')
      localStorage.removeItem('contact_form_draft')
      setForm({ name: '', email: '', company: '', message: '' })
      setTouched({ name: false, email: false, company: false, message: false })
  ...
  function inputStyle(fieldName, extra = {}) {
    return {
      ...
      padding: '11px 14px',
      background: inputBg,
      border: `1.5px solid ${borderColor(fieldName)}`,
      borderRadius: '10px',
      fontSize: '15px',
      color: text,
  ...
  const info = [
    { icon: 'mail',          color: '#2563EB', label: 'Email',     value: 'hola@serelmejor.com' },
  ...
  <meta name="twitter:image"       content="https://salesek.onrender.com/og-salesek.png" />
      ...
      <section style={{ padding: '80px 24px 64px', textAlign: 'center' }}>
        <div style={{ maxWidth: '600px', margin: '0 auto' }}>
          ...
          <p style={{ fontSize: '18px', color: textSub, lineHeight: '1.6' }}>
            {t('contact.subtitle')}
          </p>
        </div>
      </section>
      ...
      <section style={{ padding: '0 24px 100px' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1.4fr', gap: '48px', alignItems: 'start' }}>

          {/* ── Left: info cards ── */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {info.map(item => (
              <div key={item.label} style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: '16px', padding: '20px 24px', display: 'flex', alignItems: 'center', gap: '16px' }}>
                <div style={{ width: '44px', height: '44px', borderRadius: '12px', background: `${item.color}12`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <span className="material-symbols-outlined" style={{ fontSize: '22px', color: item.color }}>{item.icon}</span>
                </div>
                <div>
                  <p style={{ fontSize: '12px', fontWeight: '600', color: textSub, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '2px' }}>{item.label}</p>
                  <p style={{ fontSize: '15px', fontWeight: '500', color: text }}>{item.value}</p>
                </div>
              </div>
            ))}

            <div style={{ background: '#2563EB', borderRadius: '16px', padding: '24px', marginTop: '8px' }}>
              <span className="material-symbols-outlined" style={{ fontSize: '28px', color: 'rgba(255,255,255,0.8)', marginBottom: '12px', display: 'block' }}>rocket_launch</span>
              <h4 style={{ fontSize: '17px', fontWeight: '600', color: 'white', marginBottom: '8px' }}>
          ...
          {/* ── Right: form ── */}
          <div style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: '24px', padding: '40px' }}>
            <h2 style={{ fontSize: '22px', fontWeight: '600', color: text, marginBottom: '6px' }}>
              Envíanos un mensaje
            </h2>
            <p style={{ fontSize: '14px', color: textSub, marginBottom: '32px' }}>
            ...
            {status === 'sent' ? (
              <div style={{ textAlign: 'center', padding: '48px 24px' }}>
                ...
                <h3 style={{ fontSize: '22px', fontWeight: '600', color: text, marginBottom: '8px' }}>¡Mensaje enviado!</h3>
                <p style={{ fontSize: '15px', color: textSub, marginBottom: '24px' }}>{t('contact.sent')}</p>
                <button
                ...
                <div>
                  ...
                  <div style={{ position: 'relative' }}>
                    ...
                    {touched.message && !errors.message && form.message && (
                      <span className="material-symbols-outlined" style={{ position: 'absolute', right: '10px', top: '12px', fontSize: '18px', color: '#10B981' }}>
                        check_circle
                      </span>
                    )}
                  </div>
