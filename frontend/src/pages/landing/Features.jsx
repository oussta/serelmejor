...
function Features() {
  ...
  const extras = [
    ...
    { icon: 'translate',    color: '#F59E0B', title: 'ES / EN',               desc: 'Interfaz en español e inglés con un clic.' },
  ...
  <meta name="twitter:image"       content="https://salesek.onrender.com/og-salesek.png" />
      ...
      <section style={{ padding: '80px 24px 64px', textAlign: 'center' }}>
        <div style={{ maxWidth: '720px', margin: '0 auto' }}>
          ...
          <h1 style={{ fontSize: '44px', fontWeight: '700', letterSpacing: '-0.02em', color: text, marginBottom: '16px', lineHeight: '1.15' }}>
            Características de <span style={{ color: '#2563EB' }}>serElMejor</span>
          </h1>
          <p style={{ fontSize: '18px', color: textSub, lineHeight: '1.6', marginBottom: '40px' }}>
            Dos módulos potentes, una sola plataforma. Diseñados específicamente para negocios de 1 a 10 personas.
          </p>
          <button
            onClick={() => navigate('/register')}
            style={{ padding: '14px 32px', background: '#2563EB', color: 'white', border: 'none', borderRadius: '12px', fontSize: '15px', fontWeight: '600', cursor: 'pointer', boxShadow: '0 8px 24px rgba(37,99,235,0.35)', fontFamily: 'Plus Jakarta Sans, sans-serif' }}
          >
      ...
      {/* ── MODULES ── */}
      {modules.map((mod, idx) => (
        <section key={mod.id} style={{ padding: '80px 24px', background: idx % 2 === 0 ? (isDark ? '#080f1c' : '#ffffff') : bg }}>
          <div style={{ maxWidth: '1200px', margin: '0 auto' }}>

            {/* Module header */}
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '48px' }}>
              <div style={{ width: '56px', height: '56px', borderRadius: '16px', background: mod.bgColor, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <span className="material-symbols-outlined" style={{ fontSize: '28px', color: mod.color }}>{mod.icon}</span>
              </div>
              <div>
                <span style={{ fontSize: '12px', fontWeight: '700', color: mod.color, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{mod.tag}</span>
                <h2 style={{ fontSize: '28px', fontWeight: '600', color: text, letterSpacing: '-0.01em', marginTop: '4px' }}>{mod.title}</h2>
              </div>
            </div>

            <p style={{ fontSize: '17px', color: textSub, lineHeight: '1.7', marginBottom: '48px', maxWidth: '680px' }}>{mod.desc}</p>

            {/* Feature grid */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px' }}>
              {mod.features.map(f => (
                <div key={f.label} style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: '16px', padding: '24px' }}>
                  <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: mod.bgColor, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '16px' }}>
                    <span className="material-symbols-outlined" style={{ fontSize: '20px', color: mod.color }}>{f.icon}</span>
                  </div>
                  <h4 style={{ fontSize: '15px', fontWeight: '600', color: text, marginBottom: '8px' }}>{f.label}</h4>
                  <p style={{ fontSize: '14px', color: textSub, lineHeight: '1.6' }}>{f.desc}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section style={{ padding: '80px 24px', background: isDark ? '#080f1c' : '#ffffff' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '56px' }}>
            ...
            <p style={{ fontSize: '17px', color: textSub }}>Funcionalidades extra que marcan la diferencia.</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px' }}>
            {extras.map(e => (
              <div key={e.title} style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: '16px', padding: '24px', display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
                ...
                <div>
                  <h4 style={{ fontSize: '15px', fontWeight: '600', color: text, marginBottom: '4px' }}>{e.title}</h4>
      ...
      <section style={{ padding: '100px 24px', background: '#0F172A', textAlign: 'center' }}>
        <div style={{ maxWidth: '600px', margin: '0 auto' }}>
          <h2 style={{ fontSize: '34px', fontWeight: '700', color: '#F1F5F9', marginBottom: '16px', letterSpacing: '-0.02em' }}>
          ...
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
            <button
              onClick={() => navigate('/register')}
              style={{ padding: '14px 32px', background: '#2563EB', color: 'white', border: 'none', borderRadius: '12px', fontSize: '15px', fontWeight: '600', cursor: 'pointer', fontFamily: 'Plus Jakarta Sans, sans-serif', boxShadow: '0 8px 24px rgba(37,99,235,0.4)' }}
            >
              Empezar gratis
            </button>
            <button
              onClick={() => navigate('/precios')}
              style={{ padding: '14px 32px', background: 'transparent', color: '#94A3B8', border: '1px solid #334155', borderRadius: '12px', fontSize: '15px', fontWeight: '500', cursor: 'pointer', fontFamily: 'Plus Jakarta Sans, sans-serif' }}
            >
              Ver precios
            </button>
          </div>
