    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting && !started.current) {
          ...
          // Parse target — handles "500", "99.9", "24/7", "14"
          const isSlash  = target.includes('/')
          const isFloat  = target.includes('.')

          if (isSlash) {
            // Animate just the first number
            ...
            const timer = setInterval(() => {
              step++
              const val = (end * (step / steps)).toFixed(1)
              setDisplay(val)
              if (step >= steps) clearInterval(timer)
            }, duration / steps)
          } else {
            ...
            const timer = setInterval(() => {
              step++
              const val = Math.round(end * (step / steps))
  ...
  <meta name="twitter:image"       content="https://salesek.onrender.com/og-salesek.png" />
      ...
      <section style={{ padding: '80px 24px 64px', textAlign: 'center' }}>
        <Animate delay={0}>
          <div style={{ maxWidth: '720px', margin: '0 auto' }}>
            ...
            <h1 style={{ fontSize: '44px', fontWeight: '700', letterSpacing: '-0.02em', color: text, marginBottom: '20px', lineHeight: '1.15' }}>
              Construido por fundadores,<br />
              <span style={{ color: '#2563EB' }}>para fundadores</span>
            </h1>
            <p style={{ fontSize: '18px', color: textSub, lineHeight: '1.7', maxWidth: '580px', margin: '0 auto' }}>
        ...
        <div style={{ maxWidth: '900px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '24px' }}>
          {stats.map((s, i) => (
            <Animate key={s.label} delay={i * 100}>
              <div style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: '20px', padding: '32px 24px', textAlign: 'center', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
                <p style={{ fontSize: '40px', fontWeight: '800', color: '#2563EB', letterSpacing: '-0.03em', marginBottom: '8px' }}>
                  <Counter target={s.target} suffix={s.suffix} />
                </p>
                <p style={{ fontSize: '14px', color: textSub }}>{s.label}</p>
              </div>
            </Animate>
          ))}
        </div>
        ...
        <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '80px', alignItems: 'center' }}>
          <Animate delay={0}>
            <div>
              ...
              <h2 style={{ fontSize: '34px', fontWeight: '600', color: text, marginTop: '12px', marginBottom: '20px', letterSpacing: '-0.02em', lineHeight: '1.2' }}>
                Democratizar las herramientas de gestión empresarial
              </h2>
              <p style={{ fontSize: '16px', color: textSub, lineHeight: '1.8', marginBottom: '20px' }}>
                Las grandes empresas tienen SAP, Salesforce, Oracle. Las pequeñas empresas tienen hojas de cálculo y grupos de WhatsApp.
              </p>
              <p style={{ fontSize: '16px', color: textSub, lineHeight: '1.8', marginBottom: '32px' }}>
              ...
              <button
                ...
                style={{ padding: '12px 28px', background: '#2563EB', color: 'white', border: 'none', borderRadius: '10px', fontSize: '15px', fontWeight: '600', cursor: 'pointer', fontFamily: 'Plus Jakarta Sans, sans-serif' }}
              >
                Únete a nosotros
              </button>
            </div>
          ...
          <Animate delay={150}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              {sectors.map(item => (
                <div key={item.label} style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: '16px', padding: '24px', textAlign: 'center' }}>
                  <div style={{ width: '48px', height: '48px', borderRadius: '12px', background: `${item.color}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px' }}>
                    <span className="material-symbols-outlined" style={{ fontSize: '24px', color: item.color }}>{item.icon}</span>
                  </div>
                  <p style={{ fontSize: '24px', fontWeight: '800', color: text, marginBottom: '4px' }}>{item.num}</p>
                  <p style={{ fontSize: '13px', color: textSub }}>{item.label}</p>
                </div>
              ))}
            </div>
          </Animate>
        ...
        <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          <Animate delay={0}>
            <div style={{ textAlign: 'center', marginBottom: '56px' }}>
              <h2 style={{ fontSize: '32px', fontWeight: '600', color: text, letterSpacing: '-0.02em', marginBottom: '12px' }}>
                Nuestros valores
              </h2>
              <p style={{ fontSize: '17px', color: textSub }}>Lo que guía cada decisión que tomamos.</p>
            </div>
          </Animate>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '20px' }}>
            {values.map((v, i) => (
              <Animate key={v.title} delay={i * 100}>
                <div style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: '20px', padding: '28px', textAlign: 'center' }}>
                  <div style={{ width: '52px', height: '52px', borderRadius: '14px', background: `${v.color}12`, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
                    <span className="material-symbols-outlined" style={{ fontSize: '26px', color: v.color }}>{v.icon}</span>
                  </div>
                  <h4 style={{ fontSize: '16px', fontWeight: '600', color: text, marginBottom: '8px' }}>{v.title}</h4>
                  <p style={{ fontSize: '14px', color: textSub, lineHeight: '1.6' }}>{v.desc}</p>
                </div>
              </Animate>
            ))}
          </div>
      ...
      <section style={{ padding: '80px 24px', background: isDark ? '#080f1c' : '#ffffff' }}>
        <div style={{ maxWidth: '700px', margin: '0 auto' }}>
          <Animate delay={0}>
            <h2 style={{ fontSize: '32px', fontWeight: '600', color: text, textAlign: 'center', marginBottom: '56px', letterSpacing: '-0.02em' }}>
              Nuestra historia
            </h2>
          </Animate>
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {timeline.map((item, i) => (
              <Animate key={i} delay={i * 120}>
                <div style={{ display: 'flex', gap: '24px', paddingBottom: i < timeline.length - 1 ? '40px' : 0 }}>
                  {/* Line */}
                  ...
                  <div style={{ paddingTop: '8px' }}>
                    ...
                    <h4 style={{ fontSize: '17px', fontWeight: '600', color: text, margin: '4px 0 8px' }}>{item.title}</h4>
                    <p style={{ fontSize: '15px', color: textSub, lineHeight: '1.6' }}>{item.desc}</p>
                  </div>
                </div>
              </Animate>
            ))}
      ...
      <section style={{ padding: '80px 24px', background: bg }}>
        <div style={{ maxWidth: '500px', margin: '0 auto', textAlign: 'center' }}>
          <Animate delay={0}>
            <h2 style={{ fontSize: '32px', fontWeight: '600', color: text, marginBottom: '48px', letterSpacing: '-0.02em' }}>
              El equipo
            </h2>
            <div style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: '24px', padding: '40px', textAlign: 'center' }}>
              <div style={{ width: '80px', height: '80px', borderRadius: '50%', background: 'linear-gradient(135deg, #2563EB, #0EA5E9)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px', color: 'white', fontSize: '32px', fontWeight: '800' }}>
                Y
              </div>
              <h3 style={{ fontSize: '22px', fontWeight: '700', color: text, marginBottom: '6px' }}>Yasser</h3>
              <p style={{ fontSize: '15px', color: '#2563EB', fontWeight: '500', marginBottom: '16px' }}>Founder & Full-Stack Developer</p>
              <p style={{ fontSize: '15px', color: textSub, lineHeight: '1.7' }}>
      ...
      <section style={{ padding: '80px 24px', background: '#0F172A', textAlign: 'center' }}>
        <Animate delay={0}>
          <div style={{ maxWidth: '560px', margin: '0 auto' }}>
            <h2 style={{ fontSize: '32px', fontWeight: '700', color: '#F1F5F9', marginBottom: '16px', letterSpacing: '-0.02em' }}>
            ...
            <button
              onClick={() => navigate('/register')}
              style={{ padding: '14px 36px', background: '#2563EB', color: 'white', border: 'none', borderRadius: '12px', fontSize: '16px', fontWeight: '600', cursor: 'pointer', fontFamily: 'Plus Jakarta Sans, sans-serif', boxShadow: '0 8px 24px rgba(37,99,235,0.4)' }}
            >
              Empezar gratis
            </button>
          </div>
