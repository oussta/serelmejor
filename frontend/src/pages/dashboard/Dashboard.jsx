...
function Dashboard() {
  ...
  async function loadStats() {
    try {
      ...
      if (Array.isArray(leads)) {
        leads.slice(0, 3).forEach(l => activity.push({ id: `lead-${l.id}`, icon: 'contacts', color: '#2563EB', text: `Lead: ${l.client_name}`, status: l.status, date: l.created_at, path: `/leads/${l.id}` }))
      }
      if (Array.isArray(orders)) {
        orders.slice(0, 3).forEach(o => activity.push({ id: `order-${o.id}`, icon: 'local_shipping', color: '#F59E0B', text: `Pedido #${o.id} — ${o.product_name}`, status: o.status, date: o.created_at, path: '/orders' }))
      }
  ...
  const statCards = [
    { icon: 'contacts',      color: '#2563EB', bg: 'rgba(37,99,235,0.08)',   label: 'Leads activos',   value: stats.leads,          path: '/leads',    trend: 'pipeline comercial' },
    { icon: 'inventory_2',   color: '#10B981', bg: 'rgba(16,185,129,0.08)',  label: 'Productos',       value: stats.products,       path: '/products', trend: 'en catálogo' },
    { icon: 'local_shipping',color: '#F59E0B', bg: 'rgba(245,158,11,0.08)',  label: 'Pedidos activos', value: stats.pending_orders, path: '/orders',   trend: 'pendientes o enviados' },
    { icon: 'warning',       color: stats.low_stock > 0 ? '#F43F5E' : '#10B981', bg: stats.low_stock > 0 ? 'rgba(244,63,94,0.08)' : 'rgba(16,185,129,0.08)', label: 'Stock bajo', value: stats.low_stock, path: '/products', trend: stats.low_stock > 0 ? 'requieren reabastecimiento' : 'todo en orden' },
  ]

  const moduleCards = [
    { path: '/leads',    icon: 'contacts',      title: 'SalesFlow CRM', text: 'Gestiona tus leads y pipeline de ventas', color: '#2563EB', bg: 'rgba(37,99,235,0.06)',  stat: `${stats.leads} activos` },
    { path: '/products', icon: 'inventory_2',   title: 'StockFlow',     text: 'Control de inventario y productos',        color: '#10B981', bg: 'rgba(16,185,129,0.06)', stat: `${stats.products} productos` },
    { path: '/orders',   icon: 'local_shipping',title: 'Pedidos',       text: 'Órdenes de compra a proveedores',          color: '#F59E0B', bg: 'rgba(245,158,11,0.06)', stat: `${stats.pending_orders} activos` },
    { path: '/team',     icon: 'group',         title: 'Equipo',        text: 'Gestiona tu equipo y roles',               color: '#0EA5E9', bg: 'rgba(14,165,233,0.06)', stat: user?.role },
  ]

  const statusLabels = { new: 'Nuevo', contacted: 'Contactado', negotiating: 'Negociando', won: 'Ganado', lost: 'Perdido', pending: 'Pendiente', sent: 'Enviado', confirmed: 'Confirmado', delivered: 'Entregado' }
    ...
    <div style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', background: pageBg, minHeight: '100vh', padding: isMobile ? '16px 16px 80px' : '28px 24px' }}>
      ...
      <div style={{ marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          <button
            onClick={loadStats}
            style={{ padding: '8px 14px', background: 'none', border: `1px solid ${cardBorder}`, borderRadius: '8px', fontSize: '13px', fontWeight: '500', cursor: 'pointer', color: textSub, fontFamily: 'Plus Jakarta Sans, sans-serif', display: 'flex', alignItems: 'center', gap: '6px' }}
          >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4, 1fr)', gap: isMobile ? '10px' : '16px', marginBottom: '20px' }}>
        {statCards.map(stat => (
          <div
            ...
            style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: '14px', padding: isMobile ? '14px' : '20px', cursor: 'pointer', transition: 'box-shadow 0.2s, transform 0.2s' }}
      ...
      <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: '20px', alignItems: 'start' }}>
        ...
        <div style={{ flex: 1, width: '100%' }}>
          <p style={{ fontSize: '11px', fontWeight: '700', color: textSub, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '12px' }}>Módulos</p>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: isMobile ? '10px' : '14px' }}>
            {moduleCards.map(card => (
              <div
                key={card.path}
                onClick={() => navigate(card.path)}
                style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: '14px', padding: isMobile ? '14px' : '20px', cursor: 'pointer', transition: 'box-shadow 0.2s, transform 0.2s', position: 'relative', overflow: 'hidden' }}
                onMouseEnter={e => { e.currentTarget.style.boxShadow = '0 8px 24px rgba(0,0,0,0.1)'; e.currentTarget.style.transform = 'translateY(-2px)' }}
                onMouseLeave={e => { e.currentTarget.style.boxShadow = 'none'; e.currentTarget.style.transform = 'translateY(0)' }}
              >
        ...
        {/* Recent activity */}
        <div style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: '14px', padding: '16px', width: isMobile ? '100%' : '360px', flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {recentActivity.map(item => (
                <div
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px', background: isDark ? '#0F172A' : '#F8FAFC', borderRadius: '10px', cursor: 'pointer', transition: 'background 0.15s' }}
          ...
          <div style={{ marginTop: '14px', padding: '10px', background: 'linear-gradient(135deg, rgba(37,99,235,0.06), rgba(14,165,233,0.06))', border: `1px solid ${isDark ? 'rgba(37,99,235,0.2)' : 'rgba(37,99,235,0.15)'}`, borderRadius: '10px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '3px' }}>
              <span className="material-symbols-outlined" style={{ fontSize: '14px', color: '#2563EB' }}>hub</span>
              <p style={{ fontSize: '11px', fontWeight: '700', color: '#2563EB' }}>The Bridge activo</p>
            </div>
